-- Inquiry assignment: expose a workspace-scoped member picker and audit assignee changes.
begin;
create function public.list_workspace_members(wid uuid)
...
language sql stable security definer set search_path='' as $$
  ...
  where m.workspace_id=wid
    and exists(select 1 from public.memberships caller where caller.workspace_id=wid and caller.user_id=auth.uid())
...
begin
  ...
  if new.assignee_id is distinct from old.assignee_id then fields:=array_append(fields,'assignee_id'); end if;
