...
create table public.workspaces (id uuid primary key default gen_random_uuid(), name text not null check(length(trim(name)) between 1 and 80), currency text not null default 'RUB' check(currency='RUB'), created_by uuid not null references auth.users(id), created_at timestamptz not null default now(), unique(created_by));
...
create policy clients_insert on public.clients for insert to authenticated with check(public.can_write_workspace(workspace_id));
create policy clients_update on public.clients for update to authenticated using(public.can_write_workspace(workspace_id)) with check(public.can_write_workspace(workspace_id));
create policy inquiries_read on public.inquiries for select to authenticated using(public.is_workspace_member(workspace_id));
create policy inquiries_insert on public.inquiries for insert to authenticated with check(public.can_write_workspace(workspace_id));
create policy inquiries_update on public.inquiries for update to authenticated using(public.can_write_workspace(workspace_id)) with check(public.can_write_workspace(workspace_id));
...
revoke all on public.workspaces,public.memberships,public.clients,public.inquiries,public.notes,public.audit_events from anon,authenticated;
grant select on public.workspaces,public.memberships,public.clients,public.inquiries,public.notes,public.audit_events to authenticated;
grant insert,update on public.clients,public.inquiries to authenticated;
grant insert on public.notes to authenticated;

create function public.guard_client_write() returns trigger language plpgsql set search_path=public,pg_temp as $$
begin
  if tg_op='INSERT' then
    new.version:=1; new.created_at:=now(); new.updated_at:=new.created_at; new.archived_at:=null;
  else
    if new.id<>old.id or new.workspace_id<>old.workspace_id or new.created_at<>old.created_at then raise exception 'immutable client identity fields'; end if;
    if new.version<>old.version then raise exception 'stale client version'; end if;
    new.version:=old.version+1; new.updated_at:=now();
  end if;
  return new;
end $$;
revoke all on function public.guard_client_write() from public;
create trigger client_guard before insert or update on public.clients for each row execute function public.guard_client_write();

create function public.normalize_inquiry_insert() returns trigger language plpgsql set search_path=public,pg_temp as $$
begin
  new.status:='new'; new.version:=1; new.created_at:=now(); new.updated_at:=new.created_at;
  new.closed_at:=null; new.archived_at:=null;
  return new;
end $$;
revoke all on function public.normalize_inquiry_insert() from public;
create trigger inquiry_insert_normalize before insert on public.inquiries for each row execute function public.normalize_inquiry_insert();

create function public.audit_inquiry_insert() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
begin
  insert into public.audit_events(workspace_id,inquiry_id,actor_id,action,entity_type,entity_id,metadata)
  values(new.workspace_id,new.id,auth.uid(),'created','inquiry',new.id,jsonb_build_object('changed_fields','[]'::jsonb,'from_status',null,'to_status','new'));
  return null;
end $$;
revoke all on function public.audit_inquiry_insert() from public;
create trigger inquiry_insert_audit after insert on public.inquiries for each row execute function public.audit_inquiry_insert();

create function public.guard_and_audit_inquiry_update() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
...
create trigger inquiry_update_guard_audit before update on public.inquiries for each row execute function public.guard_and_audit_inquiry_update();
...
create function public.bootstrap_workspace(workspace_name text) returns uuid language plpgsql security definer set search_path=public,auth,pg_temp as $$
declare uid uuid:=auth.uid(); wid uuid;
begin
  if uid is null then raise exception 'authentication required'; end if;
  if not exists(select 1 from auth.users u where u.id=uid and u.email_confirmed_at is not null) then raise exception 'confirmed email required'; end if;
  if length(trim(workspace_name)) not between 1 and 80 then raise exception 'invalid workspace name'; end if;
  -- unique(workspaces.created_by) serializes concurrent attempts and guarantees one owned workspace.
